...
(function($) {
    ...
    function windowLoadInit() {
        ...
        //             /// Fro HTML start
        //             let self = $(this);
        //             let priceValue = parseFloat(self.closest('.cart_item').find('.product-price .amount').text().slice(4));
        //             let currency = self.closest('.cart_item').find('.product-price .amount').text().slice(0, 4);
        //             let inputValue = parseInt(self.val()) < 1 ? 1 : parseInt(self.val());
        //             self.closest('.cart_item').find('.product-subtotal .amount').html(`<span>${currency}</span><span>${priceValue*inputValue}</span>`);
        //             let total = shopTable.find('.cart-subtotal .amount').text().slice(4).replace(/,/g, '');
        //             let grandTotal = inputValue > 1 ? action == "minus" ? parseFloat(total) - priceValue : parseFloat(total) + priceValue : total;
        //             if (action != "") {
        //                 shopTable.closest('.shop_table').find('.cart-subtotal .amount').html(`<span>${currency}</span><span>${grandTotal}</span>`);
        //                 shopTable.closest('.shop_table').find('.order-total .amount').html(`<span>${currency}</span><span>${grandTotal}</span>`);
        //                 action = "";
        //             }
        //             /// Fro HTML end
        //             setTimeout(function() {
